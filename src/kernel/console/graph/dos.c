...
void console_window_clear(u32 color)
{
    ...
    // clear below banner
    draw_rect(0, banner_h, fb_width, fb_height - banner_h, color);
    //TODO:
    ...
    set_cursor_pos(CONSOLE_PADDING_X, banner_h);
}
...
{
    u32 char_height = font_height * font_scale;
    u32 banner_h = banner_get_height();

    // check if cursor is at bottom
    if (get_cursor_y() + char_height >= fb_height) {
        ...
        // move content up
        for (u32 y = banner_h + line_height; y < fb_height; y++) {
        ...
        // cls top line
        for (u32 y = fb_height - line_height; y < fb_height; y++) {
            for (u32 x = 0; x < fb_width; x++) {
        ...
        // Adjust cursor
        set_cursor_y(fb_height - char_height - 10);
        if (get_cursor_y() < banner_h) set_cursor_y(banner_h);
    }
}

void console_window_update_layout(void)
{
    // Adjust cursor after fsize change
    u32 banner_h = banner_get_height();
    if (get_cursor_y() < banner_h) set_cursor_y(banner_h);
}
